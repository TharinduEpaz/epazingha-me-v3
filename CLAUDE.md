...
- **"Field-ops / Modern-Warfare" theme**: design tokens (charcoal `#0d0f0d` + acid-green `#b4ff29`, light mode darkens the green to `#4c7d10`; Rajdhani + IBM Plex Mono) live in `src/styles/global.css` via a Tailwind v4 `@theme` block. Light mode overrides tokens under `:root[data-theme='light']`. Utilities: `bg-panel`, `text-accent`, `border-line`, `font-display`, `font-mono`, …. Theme toggle persists to `localStorage` (`ThemeToggle.astro`); an inline head script sets `data-theme` before paint. Tactical "deploy" motion (`[data-reveal]` IntersectionObserver + `.ops-card` hover) also lives in `global.css`. The homepage (`src/pages/index.astro`) recreates `Portfolio.dc.html`; its tab/carousel/theme interactivity is inline vanilla JS (no React on the homepage).
